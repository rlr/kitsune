...
def contributions(request, user_id):
    ...
    # TODO: We probably need to change this when we add other
    # contributions based on different models.
    #search = Sphilastic(AnswerMetricsMappingType)
    search = (AnalyzerS().es(urls=settings.ES_URLS).indexes(read_index('metrics')))
    ...
    return _merge_and_massage_data(start_date, today, data)


DAY_IN_MILLIS = 24 * 60 * 60 * 1000.0


def _merge_and_massage_data(start, end, data, spacing=DAY_IN_MILLIS):
    merged_data = []
